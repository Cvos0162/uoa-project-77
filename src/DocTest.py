...
all_tagging = accurate_section_tagging + accurate_subsection_tagging + accurate_paragraph_tagging + accurate_list_tagging + accurate_topic_tagging + accurate_comment_tagging
all_number = number_section + number_subsection + number_paragraph + number_list + number_topic + number_comment

print(str(all_tagging) + "/" + str(all_number) + " = " + str(all_tagging/all_number))
...
for tag in new_soup.findAll('section'):
    for truth in truth_soup.findAll(key=tag['key']):
        similarity_in.append(text_cleanup(tag.get('title')))
        similarity_out.append(text_cleanup(truth.get('title')))
...
#    print('{:7}   a[{}:{}] --> b[{}:{}] {!r:>8} --> {!r}'.format(tag, i1, i2, j1, j2, similarity_in[i1:i2], similarity_out[j1:j2]))
print(s.ratio())